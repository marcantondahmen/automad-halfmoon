<@ ../../elements/card_size_cache.php @>
<@ ../../elements/card_size.php @>
<div class="@{ :classContainer }">
	<div class="row">
		<@ set { :paddingTop: @{ cardImageRelativeHeight | def (75) } } @>
		<@ foreach in pagelist @>
			<div class="@{ :classCard } d-flex align-items-stretch">
				<div class="card p-0 d-flex flex-column w-full overflow-hidden">
					<@ ../../elements/set_teaser_image.php @>
					<a 
					href="@{ url }" 
					class="card-teaser"
					style="padding-top: @{ :paddingTop }%"
					>
						<@~ if @{ :imageTeaser } @>
							<img src="@{ :imageTeaser }">
						<@ else @>
							<svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" fill="currentColor" class="bi bi-file-earmark-text" viewBox="0 0 16 16">
								<path d="M5.5 7a.5.5 0 0 0 0 1h5a.5.5 0 0 0 0-1h-5zM5 9.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5zm0 2a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5z"/>
								<path d="M9.5 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4.5L9.5 0zm0 1v2A1.5 1.5 0 0 0 11 4.5h2V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h5.5z"/>
							</svg>
						<@~ end ~@>
					</a>
					<@ ../../elements/card_body.php @>
				</div>
			</div>
		<@ else @>
			<@ ../../elements/card_no_results.php @>
		<@ end @>
	</div>
</div>
<@ ../../elements/card_size_reset.php @>